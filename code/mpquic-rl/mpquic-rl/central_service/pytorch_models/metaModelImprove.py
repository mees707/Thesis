...
class ActorCritic(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_size, use_lstm=False):
        ...
        if use_lstm:
            self.l1 = nn.LSTM(input_size=num_inputs, hidden_size=LSTM_HIDDEN, num_layers=1, batch_first=True)
    ...
    def forward(self, state, lstm_memory=None):
        state = state.float().unsqueeze(0)  # Add batch dimension

        if self.use_lstm:
            state, lstm_memory = self.l1(state, lstm_memory)
